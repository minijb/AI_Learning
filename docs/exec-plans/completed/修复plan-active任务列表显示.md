...
- [x] 运行 `python .omp/skills/executing-plans/scripts/plan-active.py`，含 ≥3 个任务的计划面板中，任务垂直排列而非挤在一行
- [x] "任务:" 标签保留，每个任务以 `  图标 任务名` 格式缩进显示在标签下方
- [x] 阻塞信息、进度条、汇总面板等其他区域不受影响
- [x] 无 Rich markup 渲染错误（`✓`/`✗`/`?` 图标使用 `[success]`/`[muted]` 标签包裹）
...
- [x] **Step 1** — 修改任务列表渲染：将水平 `"  ".join(task_parts)` 改为垂直逐行输出
