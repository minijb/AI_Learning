...
> 
>     public CheckBox(IDialogMediator mediator) => _mediator = mediator;
> 
...
>     }
> 
>     public override string ToString() => $"[CheckBox] 高级模式={(IsChecked ? "启用" : "禁用")}";
> }
> 
...
> 
>     public TextBox(IDialogMediator mediator) => _mediator = mediator;
> 
>     public void SetVisible(bool visible)
>     {
>         _visible = visible;
>         Console.WriteLine($"  [TextBox] {(visible ? "显示" : "隐藏")}");
>     }
> 
>     public void SetEnabled(bool enabled)
>     {
>         _enabled = enabled;
>         Console.WriteLine($"  [TextBox] {(enabled ? "启用" : "禁用")}");
>     }
> 
>     // 模拟用户输入文本
...
>     }
> 
>     public override string ToString() => $"[TextBox] 内容=\"{Text}\"";
> }
> 
...
> 
>     public Button(IDialogMediator mediator) => _mediator = mediator;
> 
>     public void SetEnabled(bool enabled)
>     {
>         _enabled = enabled;
>         Console.WriteLine($"  [Button] {(enabled ? "启用" : "禁用")}");
>     }
> 
>     public void SetText(string text)
>     {
>         _text = text;
>         Console.WriteLine($"  [Button] 文本=\"{text}\"");
>     }
> 
>     // 模拟用户点击按钮
>     public void Click()
...
>     }
> 
>     public override string ToString() => $"[Button] 文本=\"{_text}\", 启用={_enabled}";
> }
> 
> // ============================================
> // 具体中介者：封装所有交互规则
> // ============================================
> public class SettingsDialogMediator : IDialogMediator
> {
>     private readonly CheckBox _checkBox;
>     private readonly TextBox _textBox;
>     private readonly Button _button;
> 
>     public SettingsDialogMediator(CheckBox checkBox, TextBox textBox, Button button)
>     {
>         _checkBox = checkBox;
>         _textBox = textBox;
>         _button = button;
...
>                 _button.SetText("确认");
>                 // 文本框为空时禁用按钮
>                 _button.SetEnabled(!string.IsNullOrEmpty(_textBox.Text));
>                 break;
> 
>             case "unchecked":
>                 // 用户取消勾选"启用高级模式"
>                 _textBox.SetVisible(false);
>                 _button.SetText("保存设置");
...
>                     Console.WriteLine($">>> 保存: 高级模式=启用, 值={_textBox.Text}");
>                 else
>                     Console.WriteLine($">>> 保存: 高级模式=禁用");
>                 break;
>         }
>     }
> }
...
> // Main() 模拟操作序列
> // ============================================
> Console.WriteLine("=== 中介者模式 — 设置对话框 ===\n");
> 
> // 注意：控件构造时使用占位中介者，然后在 SettingsDialogMediator 中注回
> // 实际项目中可用 DI 或两阶段初始化
> IDialogMediator? mediator = null;
> var checkBox = new CheckBox(null!);
> var textBox = new TextBox(null!);
> var button = new Button(null!);
> mediator = new SettingsDialogMediator(checkBox, textBox, button);
> 
> // 使用反射或重新构造来注入（这里简化为直接重新创建）
> // 更好的方式：两阶段初始化
> var dialog = new SettingsDialogMediator(
>     new CheckBox(null!), new TextBox(null!), new Button(null!));
> 
> // 实际可运行的版本（用两阶段注入）：
> var cb = new CheckBox(null!);
> var tb = new TextBox(null!);
> var btn = new Button(null!);
> var med = new SettingsDialogMediator(cb, tb, btn);
> 
> // 重新创建控件以注入正确的 mediator
> // 简化版：直接用内部类持有 mediator 引用
> // 以下是完整可运行版本：
> ```
> 
> ```csharp
...
> {
>     // 将接口定义、控件类、中介者嵌入此处（同上）
> 
>     public static void Run()
>     {
>         var cb = new CheckBoxImpl();
>         var tb = new TextBoxImpl();
>         var btn = new ButtonImpl();
>         var mediator = new SettingsDialogMediator(cb, tb, btn);
> 
>         // 注入中介者到各控件
...
> 
>         // 2. 勾选复选框
>         Console.WriteLine("--- 2. 勾选\"启用高级模式\" ---");
>         cb.Toggle();
>         Console.WriteLine();
> 
>         // 3. 输入文本
>         Console.WriteLine("--- 3. 输入 \"Admin\" ---");
>         tb.SetText("Admin");
>         Console.WriteLine();
> 
>         // 4. 点击按钮保存
>         Console.WriteLine("--- 4. 点击按钮保存 ---");
>         btn.Click();
>         Console.WriteLine();
> 
>         // 5. 取消勾选
>         Console.WriteLine("--- 5. 取消勾选 ---");
>         cb.Toggle();
>         Console.WriteLine();
>     }
> }
...
> }
> 
> public class CheckBoxImpl : ICheckBox
> {
>     private IDialogMediator? _mediator;
>     public bool IsChecked { get; private set; }
>     private bool _enabled = true;
> 
>     public void SetMediator(IDialogMediator m) => _mediator = m;
>     public void SetEnabled(bool enabled) { _enabled = enabled; }
> 
>     public void Toggle()
>     {
>         if (!_enabled) return;
>         IsChecked = !IsChecked;
>         Console.WriteLine($"  [CheckBox] 用户{(IsChecked ? "勾选" : "取消勾选")} → 通知中介者");
>         _mediator?.Notify(this, IsChecked ? "checked" : "unchecked");
>     }
> 
>     public override string ToString() => $"[CheckBox] 高级模式={(IsChecked ? "启用" : "禁用")}";
> }
> 
> public class TextBoxImpl : ITextBox
> {
>     private IDialogMediator? _mediator;
>     public string Text { get; private set; } = string.Empty;
> 
...
>     {
>         Text = text;
>         Console.WriteLine($"  [TextBox] 用户输入 \"{text}\" → 通知中介者");
>         _mediator?.Notify(this, "textChanged");
>     }
> }
> 
> public class ButtonImpl : IButton
> {
>     private IDialogMediator? _mediator;
>     private bool _enabled;
...
>     public void Click()
>     {
>         if (!_enabled) { Console.WriteLine("  [Button] 按钮已禁用，忽略点击"); return; }
>         Console.WriteLine("  [Button] 用户点击 → 通知中介者");
>         _mediator?.Notify(this, "click");
>     }
> }
> 
> // SettingsDialogMediator 同上（实现 IDialogMediator）
> // ...
> ```
>
> **关键设计要点：**
> - 三个控件**完全不引用彼此**——只持有 `IDialogMediator` 引用
> - `SettingsDialogMediator` 在构造函数中持有三个控件的引用，在 `Notify` 中根据事件分发
> - 控件通过 `SetMediator()` 两阶段注入中介者（打破构造循环依赖）
> - 操作序列验证了所有 5 条交互规则

> [!tip]- 练习 2 参考答案：MediatR 风格的简易中介者
>   
> ```csharp
> using System;
> using System.Collections.Generic;
...
> 
> // ============================================
> // SimpleMediatorV2 — 带管线 + 验证的中介者
> // ============================================
> public class SimpleMediatorV2 : IMediator
> {
>     private readonly IServiceProvider _serviceProvider;
> 
>     public SimpleMediatorV2(IServiceProvider serviceProvider)
>     {
>         _serviceProvider = serviceProvider;
>     }
> 
...
>         var responseType = typeof(TResponse);
> 
>         // 1. 自动验证（如果注册了 IValidator<TRequest>）
>         var validatorType = typeof(IValidator<>).MakeGenericType(requestType);
>         var validator = _serviceProvider.GetService(validatorType);
>         if (validator is not null)
>         {
>             var validateMethod = validatorType.GetMethod("Validate")!;
>             var validationResult = (ValidationResult)validateMethod.Invoke(validator, new object[] { request })!;
>             if (!validationResult.IsValid)
>             {
>                 throw new ValidationException(
>                     $"验证失败: {string.Join(", ", validationResult.Errors)}");
>             }
>         }
> 
>         // 2. 获取处理器
...
>         var handler = _serviceProvider.GetService(handlerType)
>             ?? throw new InvalidOperationException($"未注册 {handlerType.Name}");
> 
>         var handleMethod = handlerType.GetMethod("Handle")!;
> 
>         // 3. 构建管线：Pipeline[0] → Pipeline[1] → ... → Handler
>         var pipelineType = typeof(IPipelineBehavior<,>).MakeGenericType(requestType, responseType);
>         var pipelines = ((IEnumerable<object>)_serviceProvider.GetService(
...
>             handlerDelegate = () =>
>                 (Task<TResponse>)pipelineHandleMethod.Invoke(pipeline,
...
> 
> // ============================================
> // 测试：CreateUserCommand + Validator + Pipeline
> // ============================================
...
>         Console.WriteLine($"[CreateUserHandler] 创建用户 {userId}: Name={request.Name}, Email={request.Email}");
...
>     {
>         Console.WriteLine($"[LoggingPipeline] → 开始处理 {typeof(TRequest).Name}");
>         var sw = System.Diagnostics.Stopwatch.StartNew();
>         try
>         {
>             var response = await next();
>             sw.Stop();
>             Console.WriteLine($"[LoggingPipeline] ← 完成处理 {typeof(TRequest).Name}，耗时 {sw.ElapsedMilliseconds}ms");
>             return response;
>         }
>         catch (Exception ex)
>         {
>             Console.WriteLine($"[LoggingPipeline] ✗ 处理失败 {typeof(TRequest).Name}: {ex.Message}");
>             throw;
>         }
>     }
> }
...
>         => AddSingleton(typeof(TInterface), new TImpl());
> 
>     public void AddSingleton(Type interfaceType, object instance)
>     {
>         if (!_registrations.ContainsKey(interfaceType))
>             _registrations[interfaceType] = new List<object>();
>         _registrations[interfaceType].Add(instance);
> 
>         // 同时注册为 IEnumerable<T>
>         var enumerableType = typeof(IEnumerable<>).MakeGenericType(interfaceType);
>         if (!_registrations.ContainsKey(enumerableType))
>             _registrations[enumerableType] = new List<object>();
>         _registrations[enumerableType].Add(instance);
>     }
> 
...
>             if (_registrations.TryGetValue(innerType, out var services))
>             {
>                 // 构建类型化列表
>                 var listType = typeof(List<>).MakeGenericType(innerType);
>                 var list = Activator.CreateInstance(listType)!;
>                 var addMethod = listType.GetMethod("Add")!;
>                 foreach (var svc in services) addMethod.Invoke(list, new[] { svc });
>                 return list;
>             }
>             return null!;
>         }
> 
>         if (_registrations.TryGetValue(type, out var regs) && regs.Count > 0)
>             return regs[0];
>         return null!;
>     }
> 
>     public SimpleServiceProvider Build() => new(this);
> }
> 
> public class SimpleServiceProvider : IServiceProvider
> {
>     private readonly SimpleServiceCollection _collection;
>     public SimpleServiceProvider(SimpleServiceCollection col) => _collection = col;
>     public object? GetService(Type type) => _collection.GetService(type);
> }
> 
> static async Task TestMediatRV2()
> {
>     Console.WriteLine("=== MediatR 风格中介者 V2 ===\n");
> 
>     // 注册服务
>     var services = new SimpleServiceCollection();
>     services.AddSingleton<IRequestHandler<CreateUserCommand, Guid>, CreateUserHandler>();
>     services.AddSingleton<IValidator<CreateUserCommand>, CreateUserValidator>();
>     // 注册管线（IPipelineBehavior 的 IEnumerable）
>     var pipelineType = typeof(IPipelineBehavior<,>).MakeGenericType(typeof(CreateUserCommand), typeof(Guid));
>     services.AddSingleton(pipelineType, new LoggingPipeline<CreateUserCommand, Guid>());
> 
>     var sp = services.Build();
>     var mediator = new SimpleMediatorV2(sp);
> 
>     // 测试 1：合法请求
>     Console.WriteLine("--- 测试 1：合法请求 ---");
>     var validCmd = new CreateUserCommand { Name = "Alice", Email = "alice@example.com" };
>     var userId = await mediator.Send<Guid>(validCmd);
>     Console.WriteLine($"  创建用户成功: {userId}\n");
> 
>     // 测试 2：非法请求（缺少邮箱）
>     Console.WriteLine("--- 测试 2：非法请求（空邮箱） ---");
>     var invalidCmd = new CreateUserCommand { Name = "Bob", Email = "" };
>     try
>     {
>         await mediator.Send<Guid>(invalidCmd);
>     }
...
> 
>     // 测试 3：非法请求（邮箱格式错误）
>     Console.WriteLine("--- 测试 3：非法请求（邮箱格式无效） ---");
>     var badEmailCmd = new CreateUserCommand { Name = "Charlie", Email = "not-an-email" };
>     try
>     {
>         await mediator.Send<Guid>(badEmailCmd);
>     }
>     catch (ValidationException ex)
>     {
>         Console.WriteLine($"  验证失败: {ex.Message}\n");
...
>
> **关键设计要点：**
> - 管线执行顺序：`Pipeline[0]` → `Pipeline[1]` → ... → `Handler`（洋葱模型，外层先执行前处理、后执行后处理）
> - 验证在管线之前执行，验证失败直接抛 `ValidationException`，不进入管线和处理器
> - `RequestHandlerDelegate` 是一个委托链：每个管线在调用 `next()` 前做预处理，调用后做后处理（日志记录耗时即此模式）
> - 使用反射动态构造泛型类型（真正的 MediatR 用编译的委托/表达式树避免反射开销）

> [!tip]- 练习 3 参考答案（可选）：聊天室日志装饰器与事件（Decorator + Event 两种方案）
>   
> **方案 B：LoggingChatRoomDecorator（Decorator 模式）**
>   
> ```csharp
> // ============================================
> // 日志装饰器 — 包装 IChatRoom 接口
> // ============================================
> public class LoggingChatRoomDecorator : IChatRoom
> {
>     private readonly IChatRoom _inner;
> 
>     public LoggingChatRoomDecorator(IChatRoom inner) => _inner = inner;
> 
>     public void AddUser(User user)
>     {
>         Console.WriteLine($"[LOG {DateTime.Now:HH:mm:ss}] 用户 {user.Name} 准备加入");
>         _inner.AddUser(user);
>     }
> 
>     public void SendMessage(User sender, string message)
>     {
>         Console.WriteLine($"[LOG {DateTime.Now:HH:mm:ss}] {sender.Name} 发送消息: \"{message}\"");
>         _inner.SendMessage(sender, message);
>     }
> }
> 
> // 使用：
> // var chatRoom = new LoggingChatRoomDecorator(new ChatRoom());
> // var alice = new User("Alice", chatRoom);
> // 其余代码不变——User 类完全不需要修改
> ```
> 
> **方案 C：C# event 方式**
> 
...
> 
>     public MessageEventArgs(string senderName, string message, DateTime timestamp)
>     {
...
> }
> 
> public class ObservableChatRoom : IChatRoom
> {
>     private readonly ChatRoom _inner = new();
> 
>     // 暴露事件
>     public event EventHandler<MessageEventArgs>? MessageSent;
>     public event EventHandler<MessageEventArgs>? UserAdded;
> 
>     public void AddUser(User user)
>     {
>         _inner.AddUser(user);
>         UserAdded?.Invoke(this, new MessageEventArgs(user.Name, "", DateTime.Now));
>     }
> 
>     public void SendMessage(User sender, string message)
>     {
>         _inner.SendMessage(sender, message);
>         MessageSent?.Invoke(this, new MessageEventArgs(sender.Name, message, DateTime.Now));
>     }
> 
>     // 转发现有方法到内部实现
>     // （User 仍然引用 IChatRoom 接口，无需修改）
> }
> 
> // 日志订阅者
> public class MessageLogger
> {
>     public void Subscribe(ObservableChatRoom chatRoom)
>     {
>         chatRoom.MessageSent += (_, args) =>
>             Console.WriteLine($"[LOG {args.Timestamp:HH:mm:ss}] {args.SenderName} → \"{args.Message}\"");
>         chatRoom.UserAdded += (_, args) =>
>             Console.WriteLine($"[LOG {args.Timestamp:HH:mm:ss}] {args.SenderName} 加入聊天室");
>     }
> }
> 
> // 使用：
> // var chatRoom = new ObservableChatRoom();
> // var logger = new MessageLogger();
> // logger.Subscribe(chatRoom);
> // var alice = new User("Alice", chatRoom);
> // User 类完全不需要修改
> ```
>
> **方案对比分析：**
>
> | 维度 | Decorator（方案 B） | Event（方案 C） |
> |:------|:---------------------|:-----------------|
> | **耦合度** | 装饰器知道被装饰的接口 IChatRoom | 发布者和订阅者通过 Event 解耦，互相不知道 |
> | **日志执行时机** | 同步：在调用 `_inner.SendMessage()` **之前**写日志 | 同步：事件处理器在 `Invoke` 调用栈中执行 |
> | **阻塞风险** | 日志在同一个调用线程中执行——如果日志 I/O 慢，会拖慢消息转发 | 同理：`EventHandler` 是同步委托链，日志慢则全部慢 |
> | **可扩展性** | 添加新功能需新建 Decorator（可无限嵌套） | 添加新功能只需 `+=` 新订阅者，无需修改 ChatRoom |
> | **何时选用** | 日志是**消息传递流程的一部分**，必须在消息发出前/后精确执行，且要对客户端透明——Decorator 确保每次调用必经日志 | 日志是**可选的横切关注点**，可能有多个互不相关的消费者（日志、审计、监控）各自订阅——Event 更自然 |
> | **阻塞问题处理** | Decorator 内可用 `Task.Run` 异步写日志，但会引入 fire-and-forget 风险 | Event 可用 `async void` 或 `BeginInvoke` 异步——但不推荐；正确做法是事件订阅者内部自己用 Channel/Queue 异步消费 |
>
> **结论：**
> - 日志本身是同步的轻量操作（写内存或 `Console.WriteLine`）→ **Decorator** 更简单直观
> - 日志可能涉及磁盘 I/O 或远程调用 → **Event** + 订阅者内部异步队列（生产者-消费者），避免阻塞主流程
> - 多个独立消费者（日志 + 审计 + 监控）→ **Event** 更合适
> - 需要保证日志在特定时刻执行（如"消息转发前必须记录"）→ **Decorator** 提供更强的时序保证
