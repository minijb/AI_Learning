...
>
> **验证要点：**
> - 恢复后，对话历史（之前"列出项目根目录的文件"的交互）应仍在 TUI 中可见
> - 会话数据存储在 `~/.omp/agent/sessions/--<项目路径编码>--/<timestamp>_<id>.jsonl`
> - 可以用 `cat ~/.omp/agent/sessions/` 查看该目录，确认新会话文件已创建
>
> **思考题答案：** OMP 的会话持久化使用 JSONL 格式（每行一个 JSON entry），这使得追加写入高效（只需 append 行），且崩溃恢复简单（最后一行损坏只丢失一个 entry）。会话按项目 cwd 编码分组存储，确保不同项目的会话互不干扰。

> [!tip]- 练习 2 参考答案
> 操作步骤：
>
> ```text
> 1. 让 OMP 修改一个文件（如"在 src/utils.ts 中添加一个 formatDate 函数"）
> 2. 再进行第二次修改（如"给 formatDate 添加 JSDoc 注释"）
> 3. 按 Ctrl+T 打开会话树 —— 你会看到 root → 修改1 → 修改2 的线性链
> 4. 选中"修改1"节点，按 Ctrl+B 创建分支
> 5. 在新分支上做不同的修改（如"把 formatDate 改成接受 Date 对象而非字符串"）
> 6. 现在你有两条路径：
>    - 分支 A（原路径）：formatDate 接受字符串 + JSDoc
>    - 分支 B（新分支）：formatDate 接受 Date 对象
> 7. 在会话树中切换分支，用 read 查看文件验证代码确实不同
> ```
>
> **关键理解：** 分支不创建新会话——它在同一会话内创建 DAG 子图。这比 fork 更轻量：切换分支时 OMP 会"回放"该分支上的工具调用来还原代码状态。如果分支间代码冲突太大，OMP 会提示。
...
>
> **观察要点：**
> 1. OMP 会创建一个子代理（类型为 `explore`），在 TUI 中显示为一个独立的子任务
> 2. 子代理有自己的会话文件（同样存储在 `~/.omp/agent/sessions/` 下）
> 3. 子代理完成后，OMP 使用 `read agent://<子代理ID>` 读取其输出
> 4. 父会话中会引用 `agent://` URL 展示子代理的分析结果
>
> **`agent://` URL 机制：** 这是一个内部 URL 协议——`agent://<id>` 在父会话中直接解析为子代理的输出文本。父代理不需要"等待"子代理写入文件，也不需要知道子代理的会话文件路径——`agent://` 封装了这些细节。
