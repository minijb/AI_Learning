...
> [!tip]- 练习 1 参考答案
> **用 RenderDoc 测量 Overdraw 的操作指南：**
>
> 1. **打开 Quad Overdraw 视图**：
>    - 捕获一帧后，在 Texture Viewer 中点击 "Overlay" 下拉菜单
>    - 选择 "Quad Overdraw"
>    - 画面会以伪彩色显示 Overdraw 程度
>
> 2. **颜色解读**（RenderDoc 默认配色）：
>    | 颜色 | Overdraw 倍数 | 严重程度 | 行动 |
>    |------|--------------|---------|------|
>    | 深蓝/黑 | 0-1x | 理想 | 无需处理 |
>    | 青色/绿 | 2-4x | 轻度 | 可接受（不透明物体正常范围） |
>    | 黄色 | 4-8x | 中度 | 关注，寻找原因 |
>    | 红色 | 8-16x | 严重 | 需要修复 |
>    | 白色 | >16x | 紧急 | 必须立即修复 |
>
> 3. **标注红色/白色区域并诊断**：
>
>    | 区域 | 截图位置 | Overdraw 颜色 | 可能原因 | 优化方案 |
>    |------|---------|--------------|---------|---------|
>    | 粒子爆炸中心 | 屏幕中央 | 白色 (>16x) | 粒子 Quad 大量重叠（每次爆炸 15 个 Quad） | 减少粒子数 50%，缩小粒子大小 |
>    | UI 面板叠层 | 屏幕边缘 | 红色 (10x) | 3 层半透明面板 + 模糊背景叠在一起 | 合并面板为单层，背景预模糊为纹理 |
>    | 植被密集区 | 下半屏幕 | 黄色-红色 (6x) | 草地 mesh 重叠，无深度排序 | 启用 Early-Z，或减少草地密度 |
>    | 玻璃窗户 | 建筑物 | 黄色 (5x) | 透明窗户 + 室内物体 + 室外反射 | 玻璃降为不透明，用 Cubemap 替代实时反射 |
>
> 4. **量化验证**：
>    - 在问题区域捕获第二帧（关闭可疑效果后）
>    - 对比 Quad Overdraw 视图的变化
>    - 在 Profiler 中验证帧时间变化

> [!tip]- 练习 2 参考答案
> **透明排序验证 — 原理与观察：**
>
> 透明物体必须从后到前渲染，因为混合方程 `result = src * alpha + dst * (1 - alpha)` 依赖于背景颜色。
>
> ```
> 场景: 3 个半透明四边形 (A=红, B=绿, C=蓝), alpha=0.5, 深度: A 最近, C 最远
>
> 从后到前 (正确):
>   1. 画 C (蓝, 在远处)
>   2. 画 B (绿): result = 绿×0.5 + 蓝×0.5 → 蓝绿混合
>   3. 画 A (红): result = 红×0.5 + 蓝绿混合×0.5 → 红在前，透过半透明看到蓝绿
>   结果: 正确的透明度合成 ✓
>
> 从前到后 (错误):
>   1. 画 A (红)
>   2. 画 B (绿): result = 绿×0.5 + 红×0.5 → 绿在"前"但看起来在 A 的后面
>   3. 画 C (蓝): result = 蓝×0.5 + 红绿混合×0.5
>   结果: C 看起来在 B 的前面 → 完全错误的深度感知 ✗
>
> 不排序 (随机):
>   每帧不同顺序 → 颜色闪烁抖动 → 最差的体验
> ```
>
> **在引擎中验证**：
> - Unity: 修改 Shader 的 Render Queue，对比 `"Queue"="Transparent"` (默认排序) vs 手动乱序
> - 在 Scene 视图中使用 Wireframe 模式可以看到渲染顺序
>
> **关键认知**：不透明物体从前到后（利用 Early-Z），透明物体从后到前（混合顺序要求）。这是渲染管线的基础规则。

> [!tip]- 练习 3 参考答案（可选）
> **填充率 vs 带宽瓶颈分离测试设计：**
>
> | 测试 | 场景 A: 带宽瓶颈 | 场景 B: 填充率瓶颈 |
> |------|-----------------|-------------------|
> | Shader | Unlit 纹理采样（4 次 trilinear） | Perlin Noise 纯 ALU（0 次纹理采样） |
> | 每像素 ALU | ~5 | ~50 |
> | 每像素纹理采样 | 4 | 0 |
> | 主要瓶颈 | 显存带宽 | ALU 吞吐量 |
>
...
> ```
>                       1080p→540p 帧时间变化
> 场景 A (带宽瓶颈):     ~30% 降低     (分辨率和带宽需求都降为 1/4)
> 场景 B (填充率瓶颈):   ~60-70% 降低  (PS 调用降为 1/4, 且每个 PS 更便宜)
> ```
>
> **判断标准**：
> - 降分辨率后帧时间接近**线性**降低（到 1/4 分辨率 → 帧时间降到 ~1/4）→ **填充率瓶颈**
> - 降分辨率后帧时间降低**不如线性** → **带宽瓶颈**（部分开销与分辨率无关：顶点处理、Draw Call、固定管线）
>
> **实际测量代码要点**：
> ```cpp
> // 使用 GPU Timestamp Query (GL_ARB_timer_query 或 D3D12 timestamp)
> // 分别测量 PS 阶段和整个 Pass 的时间
> // 如果 PS 时间占总时间的 > 60% → 填充率瓶颈
> // 如果 PS 时间占比 < 40% → 可能带宽或其他瓶颈
> ```
