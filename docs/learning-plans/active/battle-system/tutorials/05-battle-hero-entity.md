...
-- setInitAttr() 设计为 ctor 和对象池 clear() 的双重入口，保证字段归零避免跨战斗污染
```lua
-- 六维基础属性——来自配置表 ClassLevelInfo（通过 LoadClassConnection 写入），战斗中只读不变
self.HP_INI  = 0   -- 生命基础值
self.AT_INI  = 0   -- 物攻基础值
...
```lua
-- HeroTurnComp:ctor() 初始化 — 所有标志位默认 false，由回合调度器按状态转移规则置位
self.actionIndex           = 0      -- 本轮第几次行动（从 1 开始，首次 StartTurnAction 时递增）
-- 再动时不递增 actionIndex，使得再动阶段不重复扣减技能 CD 和 Buff 回合数
...
-- 三种"额外行动"资格，优先级和结算时机不同，详见第 9 节陷阱 5
self.hasNewTurn  = false  -- 拥有"再行动"资格（结算后触发，可放主公技）
...
```lua
-- HeroMapComp:ctor() — 缓存全部初始化为无效值，首次访问时强制重算
-- calcuteX/Y 初始 -1 保证与有效坐标（≥0）不相等，isGridDirty=false 但缓存 key 天然失效
-- 缓存 key 包括坐标、移动力、攻击距离、地形引用，任一变即触发 HasValidGridDatas 返回 false
...
**格子 key 编码**（在多处出现）：
```lua
-- 将二维坐标打包为 32 位整数 key：高 16 位存 X，低 16 位存 Y
-- 使用位运算而非字符串拼接，避免 GC 分配，适合高频网格查询
...
-- class.AddComponents 按数组顺序调用 Component:ctor()，形成初始化依赖链：
-- 伤害计算 → 属性 → 技能 → 移动 → Buff → 属性 API → 地形 → 对冲 → AI → 触发 → 回合 → 调试 → 装备 → 地图
...
-- 调用父类 Entity:ctor()，初始化 id、teamType、isDied、isVisible 等基础字段
-- 父类字段由对象池 alloc 时赋值，此处仅保证继承链完整
    BattleHero.super.ctor(self)
-- 以下为 nil 的字段将在 LoadConfig() / OnEnterBattle() 时赋值，nil 表示尚未初始化
    self.battle          = nil
    self.clientEntity    = nil
    self.armyType        = CDataEnum.ArmyTag.None
    self.actionValue     = 0
    self.addActionValue  = 0
-- 行动值初始为 0，在 LoadClassInfo() 时由配置表写入；autoActionValue 供玩家托管 AI 使用
    self.autoActionValue = 0
    self.isInPerform     = false
-- isReinforcement 初始 nil（非 nil 时表示剧情援军），区别于 false（非援军但已确定）
    self.isReinforcement = nil
-- 两个延迟回调队列：delayCallbackList 在下一帧执行，delayCallbackList2 在本帧末执行
...
```lua
-- setInitAttr() 同时作为 ctor 和对象池 clear() 的入口，保证复用实例字段归零
...
-- modifyProperties 是运行期可变的属性槽，区别于 _INI 只读基准值
-- "MaxHP" 槽位初始化为 HP_INI，战斗中可通过 ModifyProperty() 改写（如血量上限变更技能）
...
```lua
-- GetMaxHP() 实时遍历所有 Buff 计算最终值，不缓存——Buff 状态每帧可能变化
...
-- 跳过已标记销毁的 Buff（isDestroy=true），避免 Buff 移除过程中重复计算
    ...
    -- 公式：先乘后加，万分比四舍五入
    -- ScaleUp.Round4 = 5000 提供四舍五入：当 (maxHp * rate) 的小数部分 ≥ 0.5 时进位
...
```lua
-- 错误：直接读 _INI 字段，跳过了 Buff 层和对冲战斗属性层的叠加
local atk = hero.AT_INI
-- _INI 是配置表裸值，不含任何运行时修改（参考第 6 节三层架构）

-- 正确：通过 HeroBuffAttrComp 暴露的 Get 方法获取经过完整叠加的最终属性值
