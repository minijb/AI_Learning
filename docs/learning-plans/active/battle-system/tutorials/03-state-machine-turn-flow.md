...
self.mainStateMgr:AddState("StateNone",
    ...
    CallbackHandler(self,"onStateNoneLeave"))
...
function Battle:onArrangeStateEnter()
    self.battleState = Enum.BattleState.Arrange
    self:SwitchFriendToSelfWhenNoAiConfig()
    self:ResetAttr() -- 重置所有战场属性，将领/士兵的 buff 属性在此计算
    self:NotifyClient(LogicEventID.SetBattleState, false, self.battleState)
end
```
...
---@field public MaxTurn       integer          -- 最大回合数，默认 99
---@field public orderList     table            -- 回合内阵营行动顺序
---@field public curOrderIndex integer          -- 当前正在行动的阵营索引
---@field public curActionTeamType Enum.Team    -- 当前行动阵营
---@field public curActionHero BattleHero       -- 当前行动英雄
---@field public turnState     TurnState        -- 整体回合状态
---@field public teamTurnState TeamTurnState    -- 阵营回合状态
---@field public operationStep integer          -- 当前回合操作序号（每次英雄行动递增）
---@field public heroActionRecord table         -- 记录每回合每英雄是否已行动
```
...
self.orderList = {
    { teamType = Enum.Team.Self,    isCompleted = false },
    { teamType = Enum.Team.Friend,  isCompleted = false },
    { teamType = Enum.Team.Enemy,   isCompleted = false }
}
```
...
HeroTurnState = {
    ...
    Stop          = 10, -- 行动彻底结束，等待 UpdateHeroTurn 回收
}
```
...
function AIAction:SetHeroTurnState()
    ...
    if hero.turnState == Enum.HeroTurnState.NewMove then
        hero.turnState = Enum.HeroTurnState.StartNewMove
    elseif hero.turnState == Enum.HeroTurnState.Continue then
        hero.turnState = Enum.HeroTurnState.StartContinue
    elseif hero.turnState == Enum.HeroTurnState.NewTurn then
        hero.turnState = Enum.HeroTurnState.None
    end
end
```
...
function BattleTurnComp:UpdateBattleTurn()
    ...
    if self.turnState == TurnState.StartTurn then
        self.teamTurnState = TurnState.None
        self:StartTurn(self.turnId + 1)
    ...
    if self.teamTurnState == TeamTurnState.StartTeamTurn then
        self.teamTurnState = TeamTurnState.None
        self.curOrderIndex = self.curOrderIndex + 1
        local order = self.orderList[self.curOrderIndex]
        local nextTeamType = order.teamType
        self:StartTeamAction(self.turnId, nextTeamType)
    elseif self.teamTurnState == TeamTurnState.StopTeamTurn then
        self.teamTurnState = TeamTurnState.None
        self:StopTeamAction(self.turnId, self.curActionTeamType)
    end
end
```
...
function BattleTurnComp:StopTurn()
    self:OnStopTurnTrigger()

    if self.isClient then
        self:NotifyClient(LogicEventID.StopTurn, true, self.turnId)
    end
    ...
    if self.turnId >= self.MaxTurn then
        ...
        self.turnState = TurnState.StartTurn
    end
end
```
...
function BattleTurnComp:UpdateHeroTurn()
    if self.turnState ~= TurnState.InTurn then return end
    if self.teamTurnState ~= TeamTurnState.InTeamTurn then return end
    ...
    if self.curActionHero ~= nil then
        if self.curActionHero.turnState == HeroTurnState_Stop then
            self.curActionHero.turnState = HeroTurnState_None

            if not self:CheckBattleResult() then
                if self:checkTeamIsCompleted(self.turnId, self.curActionHero.teamType) then
                    self.teamTurnState = TeamTurnState.StopTeamTurn
                else
                    local obj = self:getNextActionHero(self.turnId, self.curActionHero.teamType)
                    if obj ~= nil then
                        self:doStartHeroAction(self.turnId, self.curActionHero.teamType, obj)
                    end
                end
            end
        end
        self.curActionHero:UpdateTurnAction()
    end
end
```
...
local delayTime = 1
if self.isClient then
    delayTime = 1500  -- 延迟 1.5s 后才触发 buff
end
self.timerMgr:AddTimerCallback(delayTime, function()
    self:TriggerStartTurnBuffs(turnID, teamType, ...)
end)
```
