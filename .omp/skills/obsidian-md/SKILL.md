---
name: obsidian-md
description: Obsidian Markdown 语法规范 — wikilink、callout、frontmatter、标签、嵌入、尖括号/方括号包裹、Mermaid 图表等。**在本 Wiki 项目中，任何涉及创建、编写、编辑 .md 文件的操作都必须使用此 skill。** 无论用户说"写个页面"、"创建笔记"、"记录一下"、"更新文档"、"添加概念"、"完善草稿"、"写一篇关于X的wiki"还是任何会输出 markdown 内容的任务，都先触发此 skill 确保正确的 Obsidian 语法：wikilink 替代标准链接、callout 替代纯文本警告、frontmatter 必填、泛型尖括号/C# 方括号用反引号包裹、Mermaid 代码块用实际字符而非 HTML 实体等。不要只在用户明确说"Obsidian"时才触发——只要涉及写 markdown 文件就要用
inherits: markdown
user-invocable: true
---
...
> [!warning] wikilink 在表格中必须转义 `|`
> wikilink 的别名分隔符 `|`（如 `[[页面\|别名]]`）会与 Markdown 表格的列分隔符冲突，导致表格被错误拆分。在表格单元格中使用 wikilink 时，**必须**用反斜杠转义：`\|`。
>
> ```markdown
> ❌ | 模式 | [[13-facade|Facade]] |       → 解析为 4 列，表格错乱
> ✅ | 模式 | [[13-facade\|Facade]] |      → 正确 3 列
> ```
>
> 这一点**仅适用于表格内部**——表格外的 wikilink 正常使用 `[[页面|别名]]`，无需转义。
...
**Callout 块之间必须留一个空行**。如果一个 callout 紧跟在另一个 callout 之后且中间没有空行，Obsidian 会将它们渲染为一个连续块。写作时确保：
...
在参考答案模板中，**每个 `[!tip]-` 后面跟一个空行再开始下一个**——这是避免 callout 粘连的关键。

> [!important] 表格前后空行
> 表格**前后都必须有空行**。紧跟在普通文本行后面的表格（如 `**描述**：` 后直接接 `|列A|列B|`）在 Obsidian 中常常**渲染失败**或被解析为其他元素。确保表格前始终有一行空白：
>
> ```markdown
> **描述**：
>
...
Mermaid 代码块使用与普通代码块相同的语法 ` ```mermaid `，但内部是 Mermaid DSL——**不是 Markdown**。以下规则确保 Mermaid 图表在 Obsidian 中正确渲染。

#### 核心原则：Mermaid 代码块内是字面 DSL

Mermaid 解析器接收的是**原始文本**。HTML 实体（`&lt;` `&gt;` `&amp;`）不会被解码——它们会**原样显示**在图中。因此：

```mermaid
❌ B --> C[Register&lt;T&gt;]     → 图中显示 "Register&lt;T&gt;"（字面 &lt; 和 &gt;）
✅ B --> C[Register<T>]       → 图中显示 "Register<T>"
```

> [!important] 尖括号直接用 `<` `>`
> Mermaid 代码块内使用实际的 `<` `>` 字符，**不要**用 `&lt;` `&gt;` 替代。这与正文中需要反引号包裹 `IEnumerable<T>` 的规则相反——正文中的 `<T>` 会被 Markdown 解析为 HTML 标签，但代码块内是安全的字面环境。

#### 泛型类型语法

Mermaid `classDiagram` 使用 `~` 作为泛型分隔符（而非 `<` `>`）：

```mermaid
❌ class List<T> { }             → 尖括号被 Mermaid 误解析
✅ class List~T~ { }             → 渲染为 List<T>
✅ class Dictionary~K, V~ { }    → 渲染为 Dictionary<K, V>
✅ class Stack~TreeNode~T~~ { }  → 嵌套泛型 Stack<TreeNode<T>>
```

嵌套泛型的 `~~` 结尾：第一个 `~` 关闭内层泛型，第二个 `~` 关闭外层。Mermaid 按配对解析。

> [!tip] `classDiagram` vs 其他图表
> `~` 泛型语法仅用于 `classDiagram`。在 `flowchart`、`sequenceDiagram`、`graph` 等图表中，直接写 `<T>` 即可——这些图表类型不解析 `~`。

#### 标签中的特殊字符

当节点标签包含特殊字符（`()` `[]` `{}` `<>` `|` `"`）时，用引号包裹标签：

```mermaid
❌ A[IEnumerable<T>]                → 方括号嵌套导致解析失败
✅ A["IEnumerable<T>"]              → 引号包裹，正确渲染

❌ C --> D[Func<T, TResult>]        → 括号引起歧义
✅ C --> D["Func<T, TResult>"]      → 安全
```

引号包裹原则：只要标签含 `<` `>` `[` `]` `(` `)` `{` `}` `|` 中任一字符，就用双引号 `"..."` 包裹。

#### subgraph 标题

`subgraph` 的标题若含空格、中文或特殊字符（`——` `→` `、` 等），必须用双引号包裹，否则 Mermaid 解析器会报错：

```mermaid
❌ subgraph 没有中介者 —— 网状通信     → Lexical error: 中文 + 特殊字符
✅ subgraph "没有中介者 —— 网状通信"    → 引号包裹，正确渲染

❌ subgraph 对象通信                    → 纯中文也可能在某些版本报错
✅ subgraph "对象通信"                   → 一律引号最安全
```

也可使用 `subgraph ID[标题]` 语法（ID 为纯 ASCII 标识符，标题在方括号内）：

```mermaid
✅ subgraph NoMediator[没有中介者 —— 网状通信]   → ID + 标签语法，同样安全
✅ subgraph Strategy[策略模式 - 外部选择]         → 标签含空格中文也没问题
```

> [!important] subgraph 标题一律加引号或使用 ID+标签语法
> subgraph 标题含非 ASCII 字符或特殊符号时，不加引号几乎必定解析失败。养成习惯：所有含中文/空格/特殊符号的 subgraph 标题都加双引号，或使用 `subgraph ID[标题]` 语法。

> [!warning] 中文引号会导致 Mermaid 解析失败
> Mermaid 解析器将 Unicode 引号 `"` `"`（U+201C/U+201D）和 `'` `'`（U+2018/U+2019）识别为字符串分隔符，与 ASCII `"` `'` 行为相同。在 Mermaid 节点标签中使用中文引号会导致解析错误。替代方案：使用 CJK 角括号 `「」`（U+300C/U+300D）或直接省略引号。
>
> ```mermaid
> ❌ Q2{产品之间有"族"关系?}     → Parse error: 中文引号被当作字符串分隔符
> ✅ Q2{产品之间有「族」关系?}     → CJK 角括号，Mermaid 视为普通文本
> ✅ Q2{产品之间有族关系?}         → 省略引号，语义等效
> ```

#### 引号内的转义

标签用双引号包裹后，内层的双引号需要转义为 `#quot;`：

```mermaid
✅ A["Click \"Start\" to begin"]              → Markdown 层面 OK，Mermaid 可能失败
✅ A["Click #quot;Start#quot; to begin"]      → Mermaid 原生转义，可靠
```

#### 换行

节点标签内用 `<br/>` 或 `<br>` 换行（Mermaid 支持 HTML 子集）：
...
`classDiagram` 的 `note` 中用**实际换行**（不要用 `\n` — 在 Obsidian 中会原样显示）：

```mermaid
❌ note for MyClass "第一行\n第二行"          → 显示为 "第一行\n第二行"
✅ note for MyClass "第一行
    第二行"                                    → 正确换行
```
...
classDiagram
    class MyClass {
        +publicMethod()       ← + 表示 public
        -privateField         ← - 表示 private
        #protectedMethod()    ← # 表示 protected
        ~internalMethod()     ← ~ 表示 package/internal
        +staticMethod()$      ← $ 表示 static
        +abstractMethod()*    ← * 表示 abstract
    }
```
...
| 图表类型 | 语言标识 | 用途 |
|----------|---------|------|
| 流程图 | `flowchart TD` / `flowchart LR` | 决策流程、架构概览 |
| 类图 | `classDiagram` | 类型关系、接口实现 |
| 时序图 | `sequenceDiagram` | 调用时序、消息传递 |
| 旧式图 | `graph TD` / `graph LR` | 旧版流程图（推荐用 `flowchart`） |
| 状态图 | `stateDiagram-v2` | 状态机 |
| ER 图 | `erDiagram` | 实体关系 |

> [!tip] 优先使用 `flowchart` 
> `flowchart` 是 Mermaid 的新版流程图语法，比 `graph` 功能更强（支持 `subgraph` 内 `direction`、更多节点形状、更好的箭头语法）。本工作区中优先使用 `flowchart`，仅在无需新特性时允许 `graph`。
...
21. **Mermaid 代码块内使用实际字符**：Mermaid 块内使用 `<` `>` 而非 `&lt;` `&gt;`（Mermaid DSL 解析器不处理 HTML 实体）；`classDiagram` 中泛型用 `~T~` 而非 `<T>`；标签含特殊字符时用 `"..."` 包裹；`subgraph` 标题含中文/空格/特殊字符时必须用双引号包裹或使用 `subgraph ID[标题]` 语法
...
12. **交叉引用用 wikilink**：内部引用用 `[[ ]]`，不用标准 `[text](url)`；外部 URL 仍用标准链接；**表格内的 wikilink 必须转义 `|` 为 `\|`**（`[[页面\|别名]]`）— 覆盖通用链接规则
...
21. **Mermaid 代码块内使用实际字符**：Mermaid 块内使用 `<` `>` 而非 `&lt;` `&gt;`（Mermaid DSL 解析器不处理 HTML 实体）；`classDiagram` 中泛型用 `~T~` 而非 `<T>`；标签含特殊字符时用 `"..."` 包裹
